...
then
	...
	if [ X"$OPTION" = X-A ]	# Amsterdam print.
	then
		strophe -x A op8n06a.krn > $TMP/op8n06aa.krn
		exit
	fi
	if [ X$OPTION = X-D ]	# Dresden manuscript.
	then
	...
	if [ X$OPTION = X-S ]	# Selfridge-Field edition.
	then
		strophe -x S op8n06a.krn > $TMP/op8n06as.krn
		exit
	fi
	if [ X$OPTION = X-T ]	# Turin autograph.
	then
...
then
	...
	if [ X"$OPTION" = X-A ]	# Amsterdam print.
	then
		strophe -x A op8n06b.krn > $TMP/op8n06ab.krn
		exit
	fi
	if [ X$OPTION = X-D ]	# Dresden manuscript.
	then
	...
	if [ X$OPTION = X-S ]	# Selfridge-Field edition.
	then
		strophe -x S op8n06b.krn > $TMP/op8n06sb.krn
		exit
	fi
	if [ X$OPTION = X-T ]	# Turin autograph.
	then
...
then
	...
	if [ X"$OPTION" = X-A ]	# Amsterdam print.
	then
		strophe -x A op8n06c.krn > $TMP/op8n06ac.krn
		exit
	fi
	if [ X$OPTION = X-D ]	# Dresden manuscript.
	then
	...
	if [ X$OPTION = X-S ]	# Selfridge-Field edition.
	then
		strophe -x S op8n06c.krn > $TMP/op8n06sc.krn
		exit
	fi
	if [ X$OPTION = X-T ]	# Turin autograph.
	then
...
then
	...
	if [ X"$OPTION" = X-A ]	# Amsterdam print.
	then
		strophe -x A op8n07a.krn > $TMP/op8n07aa.krn
		exit
	fi
	if [ X$OPTION = X-D ]	# Dresden manuscript.
	then
	...
	if [ X$OPTION = X-S ]	# Selfridge-Field edition.
	then
		strophe -x S op8n07a.krn > $TMP/op8n07as.krn
		exit
	fi
	if [ X$OPTION = X-T ]	# Turin autograph.
	then
...
then
	...
	if [ X"$OPTION" = X-A ]	# Amsterdam print.
	then
		strophe -x A op8n07c.krn > $TMP/op8n07ac.krn
		exit
	fi
	if [ X$OPTION = X-D ]	# Dresden manuscript.
	then
	...
	if [ X$OPTION = X-S ]	# Selfridge-Field edition.
	then
		strophe -x S op8n07c.krn > $TMP/op8n07sc.krn
		exit
	fi
	if [ X$OPTION = X-T ]	# Turin autograph.
	then
...
then
	...
	if [ X$OPTION = X-S ]	# Selfridge-Field edition.
	then
		strophe -x S op8n12a.krn > $TMP/op8n12as.krn
		exit
	fi
	if [ X$OPTION = X-T ]	# Turin autograph.
	then
